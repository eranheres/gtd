...
M.show_tasks = function()
  ...
  local preview = function(ctx)
    ...
    table.insert(
      ...
      "**Status**: "
        .. (
          task.status == " " and "Open"
          or (task.status == "x" and "Completed" or (task.status == "r" and "Recurring" or task.status))
        )
    )
  ...
  local actions = {
    toggle_folder = function(picker, item)
      if not item or not item.is_folder then
        return
      end
    ...
    complete_task = function(picker, item)
      if not item or item.is_folder then
        return
      end
  ...
  Snacks.picker({
    ...
    preview = preview,
    actions = actions,
    layout = { preset = "sidebar", layout = { position = "left" } }, -- Use sidebar layout on the right side
